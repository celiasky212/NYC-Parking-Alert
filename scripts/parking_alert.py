...
def fetch_resurfacing() -> list[str]:
    ...
    try:
        ...
        hits = []
        # The schedule is an HTML table — grab every row
...
def fetch_asp(sun: date, sat: date) -> list[dict]:
    """Return list of {date, day, holiday} dicts for the week."""
    suspensions = []
    d = sun
    while d <= sat:
        ...
        if queens_header:
            for sibling in queens_header.find_next_siblings():
                if sibling.name in ("h2", "h3", "h4"):
                    break  # reached next borough section
