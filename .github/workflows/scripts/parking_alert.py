...
def fetch_resurfacing() -> list[str]:
    ...
    try:
        ...
        return [f"Could not fetch resurfacing schedule: {e}"]
...
def fetch_traffic_advisory() -> list[str]:
    ...
    try:
        ...
        return [f"Could not fetch traffic advisory: {e}"]
...
def build_email(sun: date, sat: date, resurfacing: list, asp: list, traffic: list) -> tuple[str, str, str]:
    ...
    lines = [
        "SKILLMAN / BARNETT AREA PARKING ALERT",
        f"Week of {week_str}",
        "=" * 44,
        "",
    ...
    if asp:
        lines.append(f"  {len(asp)} suspension(s) this week:")
        for s in asp:
            lines.append(f"  - {s['day']} {fmt_date(s['date'])}: SUSPENDED - {s['holiday']}")
    else:
        lines.append("ASP in effect all week. No suspensions.")
    lines += ["", "RESURFACING WORK", "-" * 16]
    if resurfacing and not resurfacing[0].startswith("Could not"):
        lines.append(f"  {len(resurfacing)} project(s) near your streets:")
        for r in resurfacing:
            lines.append(f"  - {r}")
    elif resurfacing and resurfacing[0].startswith("Could not"):
        lines += resurfacing
    else:
        lines.append("No resurfacing scheduled in your area this week.")
    lines += ["", "TRAFFIC ADVISORIES (Queens)", "-" * 26]
    if traffic and not traffic[0].startswith("Could not"):
        lines.append(f"  {len(traffic)} advisory/advisories:")
        for t in traffic:
            lines.append(f"  - {t}")
    elif traffic and traffic[0].startswith("Could not"):
        lines += traffic
    else:
    ...
    asp_items = [f"<strong>{s['day']} {fmt_date(s['date'])}</strong>: SUSPENDED - {s['holiday']}" for s in asp]
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.08)">
  ...
  {section("Alternate Side Parking", "#fff8e1", asp_items, "ASP in effect all week. No suspensions.")}
  ...
  <tr><td style="padding:20px 24px 24px">
    <p style="margin:0;font-size:12px;color:#999;border-top:1px solid #eee;padding-top:16px">
      ...
      Emergency suspensions: @NYCASP on X or NYC 311.
    </p>
  </td></tr>
</table>
</td></tr></table>
